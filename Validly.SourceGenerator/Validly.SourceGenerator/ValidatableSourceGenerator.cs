...
public class ValidatableSourceGenerator : IIncrementalGenerator
{
	...
	)
	{
		...
		var validatorClassBuilder = DeclarationBuilder
			...
			// Don't add usings here - they will be added at file level
		...
		// Build the validator class and wrap it in containing types if nested
		var validatorClass = validatorClassBuilder.Build();
		var wrappedValidatorClass = WrapInContainingTypes(validatorClass, properties.Object.ContainingTypes);

		// Build helper classes (Rules and CustomValidation) - these should NOT be wrapped in containing types
		// because they use 'file' scope which requires top-level types
		var rulesClass = rulesClassBuilder.Build();
		var customValidationInterface = customValidationsInterfaceBuilder.Build();

		// Combine all generated code
		var wrappedContent = wrappedValidatorClass.Indent()
			+ Environment.NewLine
			+ rulesClass.Indent()
			+ Environment.NewLine
			+ customValidationInterface.Indent();

		// Build using statements
		var usings = properties.Object.Usings.GetArray() ?? Array.Empty<string>();
		var usingStatements = usings.Length > 0
			? "#pragma warning disable CS0105" + Environment.NewLine
	...
	/// <summary>
	/// Wraps the generated content in containing type declarations for nested classes
	/// </summary>
	private static string WrapInContainingTypes(string content, EquatableArray<ContainingTypeInfo> containingTypes)
	{
		if (containingTypes.GetArray() is not { } types || types.Length == 0)
		{
			return content;
		}
		...
		{
			var containingType = types[i];
			var accessModifier = containingType.Accessibility switch
			{
			...
			var typeDeclaration = $"{accessModifier} partial {containingType.ClassOrRecordKeyword} {containingType.Name}";
			...
			result = $"{typeDeclaration}{Environment.NewLine}{{{Environment.NewLine}{indentedContent}{Environment.NewLine}}}";
		}
